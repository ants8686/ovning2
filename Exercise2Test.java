...
class Exercise2Test {
    ...
    public static final Recording PUNISHER = new Recording(TITLE_PUNISHER, ARTIST_PHOEBE_BRIDGERS, 2021, TYPE_CD, Set.of(GENRE_JAZZ));
    public static final Recording WKM = new Recording(TITLE_WHAT_KINDA_MUSIC, ARTIST_TOM_MISCH_YUSSEF_DAYES, 2021, TYPE_LP, Set.of("Soul"));
    ...
    void _speedTest1() {
        assertTimeoutPreemptively(Duration.ofSeconds(10), () -> {
            ...
            sut.getGenres();
        }, "Timeout... klassen Searcher kunde inte instansieras tillräckligt snabbt, vilket tyder på att någon del använder datastrukturer fel.");
    }
    ...
    void _speedTest2() {
        ...
        }, "Timeout... testerna kunde inte utföra metoden offerHasNewRecordings tillräckligt snabbt, vilket tyder på att någon del använder datastrukturer fel.");
        ...
        }, "Timeout... testerna kunde inte utföra metoderna (förutom offerHasNewRecordings & numberOf*) tillräckligt snabbt, vilket tyder på att någon del använder datastrukturer fel.");

        assertTimeoutPreemptively(Duration.ofSeconds(5), () -> {
            sut.numberOfArtists();
            sut.numberOfGenres();
            sut.numberOfTitles();
        }, "Timeout... testerna kunde inte utföra metoderna numberOf* tillräckligt snabbt, vilket tyder på att någon del använder datastrukturer fel.");
    }
    ...
    void testClassMembers() {
        ...
        long countOfApprovedMaps = Arrays.stream(sut.getClass().getDeclaredFields())
                .filter(field -> Map.class.isAssignableFrom(field.getType()))
                .filter(Exercise2Test::isMapWithCorrectTypes)
                .count();

        assertTrue(countOfApprovedMaps >= 2, "Klassen innehåller inte minst två avbildningar med Recording eller samling av Recording som värde.");
    }
    ...
    void testGetGenres() {
        ...
        Collection<String> expected = Set.of("Rock", "Stage & Screen", "Folk", "Electronic", "World", "Blues", "Funk", "Pop", "Jazz", "Soul", "Latin", "Country", "Reggae", "Classical", "Hip Hop");
    ...
    void testGetRecordingsByArtistOrderedByYearAsc() {
        ...
        var msg = String.format("Letade efter titlarna %s i samlingen %n%s%n men hittade inte alla titlar.%n", expected, recordings);
    ...
    void testGetRecordingsByGenre(String genre, int expected) {
        ...
        assertEquals(expected, actual, "Genren " + genre + " förväntades ha " + expected + " skivor men hade " + actual + ".");
    }

    @ParameterizedTest
    @Order(80)
    @DisplayName("Testar att getRecordingsByGenreAndYear ger rätt svar för olika kombinationer.")
    @CsvSource({"Jazz, 1950, 1960, 7", "Pop, 1990, 2000, 9", "Rock, 1970, 1980, 150", "Country, 2010, 2021, 0"})
    ...
    void testOfferHasNewRecordings() {
        ...
        var offered = List.of(
                new Recording(TITLE_GIANT_STEPS, ARTIST_JOHN_COLTRANE, 1959, TYPE_LP, Set.of(GENRE_JAZZ)), punisher);
    ...
    @Test
    @Order(130)
    @DisplayName("Testar metoden optionalGetRecordingsByArtistOrderedByTitleAsc om den är implemeterad.")
    ...
    void testOptionalGetRecordingsFrom() {
        ...
        assumingThat(result != null, () -> {
                    ...
                    var third = iter.next().getTitle(); //sketches
                    var actual = Set.of(first, second, third);
                    var expected = Set.of("Muddy Waters at Newport 1960", "At Last!", TITLE_SKETCHES_OF_SPAIN);
                    assertEquals(expected, actual, "Fel objekt i samlingen.");
                }
        );
    }
    ...
    void testOptionalGetRecordingsFrom(int year, int expected) {
        ...
        assumingThat(result != null, () -> {
            var msg = String.format("Det förväntades finnas %d skivor från %d men det fanns %d.", expected, year, result.size());
    ...
    void testUnmodifiableResults() {
        ...
        assertThrows(UnsupportedOperationException.class, () -> sut.offerHasNewRecordings(Collections.emptyList()).add(null), String.format(message, "offerHasNewRecordings"));
        assertThrows(UnsupportedOperationException.class, () -> sut.getRecordingsAfter(1980).add(null), String.format(message, "getRecordingsAfter"));
        assertThrows(UnsupportedOperationException.class, () -> sut.getRecordingsByArtistOrderedByYearAsc(ARTIST_JOHN_COLTRANE).add(null), String.format(message, "getRecordingsByArtistOrderedByYearAsc"));
        assertThrows(UnsupportedOperationException.class, () -> sut.getRecordingsByGenre(GENRE_JAZZ).add(null), String.format(message, "getRecordingsByGenre"));
        assertThrows(UnsupportedOperationException.class, () -> sut.getRecordingsByGenreAndYear(GENRE_JAZZ, 1960, 1965).add(null), String.format(message, "getRecordingsByGenreAndYear"));

        assumingThat(sut.optionalGetRecordingsByArtistOrderedByTitleAsc(ARTIST_JOHN_COLTRANE) != null,
                () -> assertThrows(UnsupportedOperationException.class, () -> sut.optionalGetRecordingsByArtistOrderedByTitleAsc(ARTIST_JOHN_COLTRANE).add(WKM), String.format(message, "optionalGetRecordingsByArtistOrderedByTitleAsc"))
        );

        assumingThat(sut.optionalGetRecordingsBefore(1970) != null,
                () -> assertThrows(UnsupportedOperationException.class, () -> sut.optionalGetRecordingsBefore(1970).add(null), String.format(message, "optionalGetRecordingsBefore"))
        );

        assumingThat(sut.optionalGetRecordingsFrom(1970) != null,
                () -> assertThrows(UnsupportedOperationException.class, () -> sut.optionalGetRecordingsFrom(1970).add(null), String.format(message, "optionalGetRecordingsFrom"))
        );
    }
}
